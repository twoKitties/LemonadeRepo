...
namespace NiobiumStudios
{
    ...
    public class DailyRewardsInterface : MonoBehaviour
    {
        ...
        public GameObject[] placeHolders;           // Array of rect transform posiiton for every reward
        ...
        private void InitializeDailyRewardsUI()
        {
            for (int i = 0; i < dailyRewards.rewards.Count; i++)
            {
                ...
                dailyRewardUI.transform.SetParent(placeHolders[i].transform);
                //dailyRewardGo.transform.position = placeHolders[i].transform.position;
        ...
        public void SnapToReward()
        {
            ...
            Debug.Log("lastRewardIdx " + lastRewardIdx);
            Debug.Log("dailyRewardsUI.Count " + dailyRewardsUI.Count);
            if (lastRewardIdx > 0)
            {
                //var target = dailyRewardsUI[lastRewardIdx].GetComponent<RectTransform>();
                var c = new Color(0, 0, 0, 0.15f);
                dailyRewardsUI[lastRewardIdx].imageRewardBackground.color = c;
